  ...
  return async function interactions(ctx, next) {
    ...
    if (interaction) {
      ...
      ctx.cookies.set(
        ...
        Object.assign({ path: url.parse(destination).pathname }, cookieOptions, { samesite: 'none' }),
      );

      ctx.cookies.set(
        provider.cookieName('resume'), ctx.oidc.uuid,
        Object.assign({ path: url.parse(returnTo).pathname }, cookieOptions, { samesite: 'none' }),
      );
