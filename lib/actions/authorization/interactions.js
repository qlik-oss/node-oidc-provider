...
module.exports = (provider) => {
  ...
  return async function interactions(ctx, next) {
    ...
    if (interaction) {
      ...
      const interactionCookieProps = Object.assign({ path: url.parse(destination).pathname }, cookieOptions, { samesite: 'none' });
      // eslint-disable-next-line no-console
      console.log('--AB: setting interactionCookieProps: ', interactionCookieProps);
      ctx.cookies.set(
        provider.cookieName('interaction'), ctx.oidc.uuid,
        interactionCookieProps,
      );

      const resumeCookieProps = Object.assign({ path: url.parse(returnTo).pathname }, cookieOptions, { samesite: 'none' });
      // eslint-disable-next-line no-console
      console.log('--AB: setting resumeCookieProps: ', resumeCookieProps);
      ctx.cookies.set(
